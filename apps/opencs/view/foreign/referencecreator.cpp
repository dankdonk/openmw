...
std::string CSVForeign::ReferenceCreator::getId() const
{
    return mId;
}

void CSVForeign::ReferenceCreator::configureCreateCommand (CSMWorld::CreateCommand& command) const
{
    // Set cellID
    int cellIdColumn =
    ...
    int refNumColumn = dynamic_cast<CSMWorld::IdTable&> (
        *getData().getTableModel (CSMWorld::UniversalId::Type_ForeignReferences)).
        findColumnIndex (CSMWorld::Columns::ColumnId_RefNum);

    command.addValue (refNumColumn, getRefNumCount());
}

void CSVForeign::ReferenceCreator::pushCommand (std::auto_ptr<CSMWorld::CreateCommand> command,
    const std::string& id)
{
    // get the old count
    ...
    CSMWorld::IdTable& cellTable = dynamic_cast<CSMWorld::IdTable&> (
        *getData().getTableModel (CSMWorld::UniversalId::Type_ForeignCells));
...
int CSVForeign::ReferenceCreator::getRefNumCount() const
{
    std::string cellId = mCell->text().toUtf8().constData();
...
CSVForeign::ReferenceCreator::ReferenceCreator (CSMWorld::Data& data, QUndoStack& undoStack,
...
void CSVForeign::ReferenceCreator::reset()
{
...
std::string CSVForeign::ReferenceCreator::getErrors() const
{
...
void CSVForeign::ReferenceCreator::focus()
{
    mCell->setFocus();
}

void CSVForeign::ReferenceCreator::cellChanged()
{
    update();
}

void CSVForeign::ReferenceCreator::cloneMode(const std::string& originId,
...
CSVWorld::Creator *CSVForeign::ReferenceCreatorFactory::makeCreator (CSMDoc::Document& document,
