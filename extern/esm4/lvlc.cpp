...
ESM4::LeveledCreature::LeveledCreature() : mScript(0), mTemplate(0), mChanceNone(0), mLvlCreaFlags(0)
{
    mEditorId.clear();
}
        ...
        switch (subHdr.typeId)
        {
            case ESM4::SUB_EDID: // Editor name or the worldspace
            {
                if (!reader.getZString(mEditorId))
                    throw std::runtime_error ("LVLC EDID data read error");
                break;
            }
            case ESM4::SUB_SCRI:
            {
