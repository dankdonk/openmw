        ...
        switch (subHdr.typeId)
        {
            case ESM4::SUB_EDID: // Editor name or the worldspace
            {
                if (!reader.getZString(mEditorId))
                    throw std::runtime_error ("KEYM EDID data read error");
                break;
            }
            case ESM4::SUB_FULL:
            {
            ...
            case ESM4::SUB_MODL:
            {
                if (!reader.getZString(mModel))
                    throw std::runtime_error ("KEYM MODL data read error");
            ...
            case ESM4::SUB_ICON:
            {
                if (!reader.getZString(mIcon))
                    throw std::runtime_error ("KEYM ICON data read error");
                break;
            }
            case ESM4::SUB_DATA:
            {
