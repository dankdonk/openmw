...
#include <components/nif/collision.hpp> // FIXME
...
public:
    ...
    std::map<size_t, size_t> mAnimatedRaycastingShapes;
    ...
    bool mIsRagdoll;
    std::map<size_t, btCollisionShape*>  mShapes; // key is recIndex of the bhkRigidBody
    std::map<size_t, std::vector<std::pair<size_t, size_t> > > mJoints; // one or more joint per bhkRigidBody
    std::map<size_t, btRigidBody::btRigidBodyConstructionInfo> mRigidBodyCI;
    std::map<std::pair<size_t, size_t>, Nif::RagdollDescriptor> mNifRagdollDesc;
    std::map<size_t, Ogre::Vector3> mShapeTransMap; // key is recIndex of the bhkRigidBody

    // Does this .nif have an autogenerated collision mesh?
    bool mAutogenerated;

    Ogre::Vector3 mShapeTrans; // FIXME: temporary storage
